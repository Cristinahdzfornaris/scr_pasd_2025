...
class ModelRegistryActor:
    ...
    def delete_dataset_models(self, dataset_name):
        if dataset_name in self.registered_models:
            ...
            print(f"[{self.__class__.__name__}] Modelos para '{dataset_name}' eliminados.")
            return True
        return False
...
def run_complete_training_job(dataset_name: str, df: pd.DataFrame, target_column: str):
    ...
    registry_actor = ModelRegistryActor.options(name="model_registry", get_if_exists=True, max_restarts=-1, lifetime="detached", resources={"is_head_node": 1}).remote()
    ...
    for model_config in model_configurations:
        ...
        if result is not None:
            ...
            registry_actor.register_model.remote(dataset_name, model_type, result_ref)
            successful_count += 1
    
    final_message = f"Trabajo completado. Se entrenaron y registraron {successful_count} de {len(model_configurations)} modelos para '{dataset_name}'."
